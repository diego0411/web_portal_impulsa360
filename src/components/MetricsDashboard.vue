...
function plazaRegistro(item) { return texto(item.ciudad_activacion) || texto(item.plaza) || 'Sin plaza' }
function liderRegistro(item) { return texto(item.lider_nombre) || texto(item.lider) || texto(item.nombre_lider) || '' }
...
const opciones = computed(() => {
  const plazas = new Set(), activadores = new Set(), tipos = new Set(), lideres = new Set()
...
const dashboard = computed(() => {
  ...
  return { rows, points, kpis, porDia: ranking(maps.dia).sort((a, b) => a.label.localeCompare(b.label)), porPlaza: ranking(maps.plaza), topActivadores: ranking(maps.activador, 10), porTipo: ranking(maps.tipo), topLideres: ranking(maps.lider, 10) }
})
...
function limpiarFiltros() { filtroDesde.value = ''; filtroHasta.value = ''; filtroPlaza.value = ''; filtroActivador.value = ''; filtroTipo.value = ''; filtroLider.value = '' }
